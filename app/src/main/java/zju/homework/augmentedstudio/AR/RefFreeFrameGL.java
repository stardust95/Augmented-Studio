...
public class RefFreeFrameGL
{
    ...
    String textureNames[] = {
            "viewfinder_crop_marks_portrait.png",
            "viewfinder_crop_marks_landscape.png" };
    int[] textures;
    ...
    public RefFreeFrameGL(ARSceneActivity activity,
                          ARApplicationSession session)
    {
        ...
        textures = new int[TEXTURE_NAME.TEXTURE_COUNT];
    ...
    boolean init(int screenWidth, int screenHeight)
    {
        ...
        if ((shaderProgramID = Util.createProgramFromShaderSrc(
...
//        }
    ...
    void getTextures()
    {
        double max = 0.1;
        for (int i = 0; i < TEXTURE_NAME.TEXTURE_COUNT; i++)
            textures[i] = Texture.loadTextureFromApk(textureNames[i], Util.getAsset());
    }
    ...
    void renderViewfinder()
    {
        ...
        if (isActivityPortrait
                && textures[TEXTURE_NAME.TEXTURE_VIEWFINDER_MARKS_PORTRAIT] != 0)
        {
            GLES20.glActiveTexture(GLES20.GL_TEXTURE0);
            GLES20
                    .glBindTexture(
                            GLES20.GL_TEXTURE_2D,
                            textures[TEXTURE_NAME.TEXTURE_VIEWFINDER_MARKS_PORTRAIT]);
        } else if (!isActivityPortrait
                && textures[TEXTURE_NAME.TEXTURE_VIEWFINDER_MARKS] != 0)
        {
            GLES20.glActiveTexture(GLES20.GL_TEXTURE0);
            GLES20.glBindTexture(GLES20.GL_TEXTURE_2D,
                    textures[TEXTURE_NAME.TEXTURE_VIEWFINDER_MARKS]);
        }
